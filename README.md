...
任务触发器为“登录时 + 每 30 分钟”，要求网络可用。控制器使用实时搜索、只读沙箱和 `high` 推理强度；若 `codex.exe` 不在 PATH，可在用户环境中设置 `CODEX_EXECUTABLE` 为完整路径。`CODEX_REASONING_EFFORT` 可改为 `medium` 或 `xhigh`。卸载任务运行 `scripts/uninstall-windows-task.ps1`。
...
- CLI 当前没有可依赖的“运行中到 80,000 token 自动中止”开关。项目通过最多 90 候选、16 组搜索、60 页面和严格输出长度控制输入，并读取 CLI 可用的 usage 记录；测得超过 80,000 时拒绝发布。这个限制能阻止超额内容上线，但无法追回该次已经消耗的额度。
